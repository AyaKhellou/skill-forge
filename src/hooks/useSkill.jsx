...
import { deleteSkill, getSkill, updateSkill } from "../services/firestore";
...
export default function useSkill(goalId, skillId) {
    ...
    useEffect(() => {
        ...
        setError(null);
        const unsubscribe = getSkill(user.uid,goalId, skillId, (data) => {
            setSkillData(data);
            setLoadingSkill(false);
        }, (error) => {
            setError(error);
    ...
    async function updateSkillData(dataToUpdate) {
        if (!user?.uid || !skillId || !goalId) return;
        await updateSkill(user.uid, goalId, skillId, dataToUpdate);
    }
